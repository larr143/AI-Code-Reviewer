...
class LoadingPage(ttk.Frame):
    """ The page that displays while ChatGPT processes. 

    The loading page uses queue and threading to keep the 
    mainloop alive by running a loop that checks the progress of 
    chatgpt and displays the completion %.
    ...
    def __init__(self, parent, controller):
        ...
        tk.Frame.__init__(self, parent)
        self.controller = controller
        self.loading_label = ttk.Label(self, text="Processing...")
        self.loading_label.pack(side="top",expand=True)
        self.frame_style()
            
    def start_processing(self):
        """
        When called initiates the queue and threading for the ChatGPT 
        Threaded class. 
        """
    ...
    def process_queue(self):
        """runs a loop to check and display progress of ChatGPT
        """
        try:
    ...
    def frame_style(self):
        ...
        self.loading_label.config(font=("Times New Roman", 18),
            foreground="#F573C8", background="#121841")
...
class ChatGptThreaded(threading.Thread): 
    """ChatGptThreaded _summary_
    
    A threaded class that talks to ChatGPT and reports progress then a 
    tuple when finished that contains the comments from ChatGPT and 100 
    for progress. 
    
    Attributes:
        queue (int): The queue the function is going to report to
        comments (list): A list that contains the output from Pylint.
        code (String): Contains a string that holds the users code. 
    """
